...
export class Formatter {
  ...
  static rainbowText(text: string): string {
    ...
    while (letter_idx < letter.length) {
      if (letter[letter_idx] !== " ")
        newMessage += `${rainbowCode[letter_idx % rainbowLength]}${
          letter[letter_idx]
        }`;
      else newMessage += " ";
      letter_idx++;
   ...
   */
  static thousandsSeparator(value: number): string | undefined {
   ...
   */
  static metricNumbers(value: number): string | number {
   ...
   */
  static toMilliseconds(value: string): number {
