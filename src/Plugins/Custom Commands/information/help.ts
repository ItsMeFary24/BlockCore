...
CommandBuilder.Build({
  ...
  onExecute: async ({
    ...
    const check_perms = (perms: string[], cmd_perms: string[]): boolean =>
      cmd_perms.length === 0 || cmd_perms.some((perm) => perms.includes(perm));

    const command_exist = is_registered(input_command as string);
    if (
      (!Validator.isNumber(input_command) && !command_exist) ||
      (command_exist &&
    ...
    const target_page =
      (input_command as number) > max_page
        ? max_page
        : (input_command as number);
    ...
    messages += sorted_cmds
      .map((item) => {
        ...
        return `${categoryLine} §f${command_prefix}${item.register.name} §7- ${
          item.register.usage[0] || ""
        }`;
      })
    ...
    if (remaining_items.length > 0)
      messages += `\n\n§eTo view the next page, use §f${command_prefix}help ${
        target_page + 1
      }`;
