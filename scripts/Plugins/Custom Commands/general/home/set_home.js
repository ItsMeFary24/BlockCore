...
CommandBuilder.Build({
    ...
    onExecute: ({ sender, getInput }) => {
        ...
        const sethome_limit = player
            .getTags()
            .find((tag) => Object.keys(CustomCommandConfig.set_home_max).includes(tag))
            ? CustomCommandConfig.set_home_max[player
                .getTags()
                .find((tag) => Object.keys(CustomCommandConfig.set_home_max).includes(tag))]
        ...
        if (existing_home.length >= sethome_limit) {
            player.sendMessage(`§a[§eHome§a] §cYou've reached your limit of §e${sethome_limit} §chomes.`);
            return;
        }
        if (!inputted_name) {
        ...
        if (existing_home.some((home) => home.includes(`home-${inputted_name}`))) {
            player.sendMessage(`§a[§eHome§a] §cA home named §e${inputted_name} §calready exists.`);
            return;
        }
        home_db.set(`home-${inputted_name}`, {
            pos: new Vec3D(player.location.x, player.location.y, player.location.z),
        });
        player.sendMessage(`§a[§eHome§a] Successfully set new home: §e${inputted_name}§a.`);
    },
});
