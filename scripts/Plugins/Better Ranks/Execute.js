...
BeforeEvents.on("msg_send", (params) => {
    ...
    const highestRank = tags.reduce((highest, tag) => {
    ...
    return world.sendMessage(highestRank.format
...
Interval.Always(() => {
    ...
    while (entity_idx < allPlayer.length) {
        ...
        player.nameTag = `${rankConfig.AbovePlayerDisplay[highestRank.name.toLocaleLowerCase()].format
            .replaceAll("{rank_name}", highestRank.name)
